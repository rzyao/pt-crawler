...
```
持久化输出（推荐挂载卷，并与 `/config/config.yaml` 中路径对应；切勿覆盖 `/app` 以免应用代码被隐藏）：
```bash
docker run -d -p 8800:8000 \
  --name pt-crawler \
  -v /user/pt-crawler/config:/config \
  -v /user/pt-crawler/torrents:/app/torrents \
  -v /user/pt-crawler/logs:/app/logs \
  pt-crawler
```
容器默认启动命令：
```
...
- 服务日志级别可通过 `/config/config.yaml` 中的 `log_level` 调整（如 `INFO`、`DEBUG`）
...
- 如需文件日志，设置 `/config/config.yaml`：`log_file: /logs/crawler.log` 并挂载目录：
