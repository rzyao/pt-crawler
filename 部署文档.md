...
删除镜像：
```bash
docker rmi pt-crawler
```
运行容器：
```bash
docker run -p 8800:8000 pt-crawler
```
持久化输出（推荐挂载卷，并与 `/pt-crawler/config/config.yaml` 中路径对应；切勿覆盖 `/pt-crawler` 以免应用代码被隐藏）：
```bash
docker run -d -p 8800:8000 \
  --name pt-crawler \
  -v /user/pt-crawler/config:/pt-crawler/config \
  -v /user/pt-crawler/torrents:/torrents \
  pt-crawler
```
容器默认启动命令：
```
...
- 服务日志级别可通过 `/pt-crawler/config/config.yaml` 中的 `log_level` 调整（如 `INFO`、`DEBUG`）
...
- 如需文件日志，设置 `/pt-crawler/config/config.yaml`：`log_file: /logs/crawler.log` 并挂载目录：
...
## 常见问题排查
- 服务无法启动：检查 `/pt-crawler/config/config.yaml` 的 MySQL 连接参数是否正确；端口冲突时修改 `port` 或 `uvicorn` 启动端口
