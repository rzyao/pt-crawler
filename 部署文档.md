...
```
持久化输出（如需挂载数据/下载目录，可选，仅挂数据目录，不挂配置目录）：
```bash
...
- 服务日志级别可通过根目录 `config.yaml` 中的 `log_level` 调整（如 `INFO`、`DEBUG`）
...
- 如需文件日志，设置根目录 `config.yaml`：`log_file: /logs/crawler.log` 并挂载目录：
...
## 常见问题排查
- 服务无法启动：检查根目录 `config.yaml` 的 MySQL 连接参数是否正确；端口冲突时修改 `port` 或 `uvicorn` 启动端口
